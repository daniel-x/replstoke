...
replstoke can act in one of three modes, client, server, or general. Each mode has
a set of options which are allowed in this mode. If the --server option is
present, replstoke is started in server mode. If the --client option is present,
it acts in client mode. If neither of the two are present, it's in general
mode.

Server mode options:
  -s, --server                  run in server mode, i.e. keep running and wait
                                for clients to connect. only -s or -c may be
                                given, but not both.
  ...
  -r, --restart                 restart the REPL if the REPL process ended for
                                any reason
      --restart-on-client-dc    if a client disconnects after its input was
                                forwarded but before the REPL responded, and the
                                REPL then fails to finish within --timeout,
                                restart the REPL. requires --timeout to take
                                effect.
  ...
  -m, --end-marker-stdout=M     success end-of-response marker the server watches
      ...
      --strip-out-marker        drop the matched stdout marker from what is
                                forwarded to the client.
      --strip-err-marker        drop the matched stderr marker from what is
                                forwarded to the client.
      --timeout=SECONDS         after a client disconnects mid-request, how long
...
Client mode options:
  ...
  -m, --end-marker-stdout=M     success end-of-response marker on the REPL's
      ...
      --error-marker-stderr=M   error end-of-response marker on stderr; a match
                                ...
                                default: "error".
      --strip-out-marker        when specified, the matched stdout marker is not
                                included in the client's output.
      --strip-err-marker        when specified, the matched stderr marker is not
                                included in the client's output.
...
- If a client disconnects after its input was forwarded but before the REPL finished the response, the server keeps the REPL reserved (accepting no other client) until the REPL finishes. With both --timeout and --restart-on-client-dc set, a REPL that does not finish within --timeout is considered wedged and is restarted using the same SIGTERM→SIGKILL teardown as --kill; otherwise the server waits for the REPL to finish on its own.
...
- The server permits at most one client to be active. If a client is already connected and another client tries to connect, then the server does not accept the new client's connection. The same applies while the REPL is still busy with a previous request whose client has already disconnected. (A client that connects while the REPL is still booting is not rejected; it is accepted and its input is held until the REPL is ready.)
